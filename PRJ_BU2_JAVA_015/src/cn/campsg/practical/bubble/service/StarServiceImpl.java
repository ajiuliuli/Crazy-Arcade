...
public class StarServiceImpl implements StarService {
	...
	private void Random(StarList stars) {
		for (int i = 0; i <= 10; i++) {
				stars.get(i).setPosition(new Position());
		}
	}
	...
	public StarList getVMovedStars(StarList clearStars, StarList currentStarList) {
		...
		while (keys.hasNext()) {
			...
			for (int row = starPosition; row >= 0; row--) {
				...
				// 设置泡泡糖上下移动距离
				mStar.getMovedPosition().setRow(mStar.getPosition().getRow() + span);
				// 待修改
				mStar.getMovedPosition().setColumn(mStar.getPosition().getColumn());
				// 保存待移动的泡泡糖
				moveStars.add(mStar);
		...
		if (logger.isDebugEnabled())
			logger.debug("待移动泡泡糖内存列表（垂直移动方向的）:" + moveStars + "待移动泡泡糖个数为：" + moveStars.size());
	...
	public StarList getHMovedStars(StarList currentStarList) {
		...
		for (int column = starPosition; column < StarService.MAX_COLUMN_SIZE; column++) {
			...
			for (int row = (StarService.MAX_ROW_SIZE - 1); row >= 0; row--) {
				...
				// 待修改
				mStar.getMovedPosition().setColumn(mStar.getPosition().getColumn() - span);
				// 保存待移动的泡泡糖
				moveStars.add(mStar);
			}
		}
