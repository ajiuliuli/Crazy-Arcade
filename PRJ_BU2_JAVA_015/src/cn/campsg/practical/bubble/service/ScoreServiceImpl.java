...
public class ScoreServiceImpl implements ScoreService {
	...
	public int getScoreByStars(int stars) {
		...
		Score = LOWER_SCORE * (int) Math.pow(stars, 2);
	...
	public int getAwardScore(int leftStarNum) {
		...
		if (leftStarNum < AWARD_LIMIT)
			return LOWER_AWARD_SCORE * (int) Math.pow((AWARD_LIMIT - leftStarNum), 2);
	...
	public boolean isChangeLevel(int score) {
		...
		if (score >= configuration.getScore().getLevelScore())
			return true;
		else
			return false;
	}
	...
	public boolean isNoticePassLevel(int currentLevel, int score) {
		...
		//判断关卡结束后是否需要继续显示关卡通过 
		if (mLevelCounter != currentLevel)
			return false;
		else {
			mLevelCounter++;

			if (isChangeLevel(score) == false)
				return false;
			else
				return true;
		}
